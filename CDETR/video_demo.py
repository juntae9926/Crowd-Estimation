...
def main(args):
    ...
    out_folder = './out_'+ args['video_path'].split('/')[-1].split('.')[0]
    image_list = os.listdir(os.path.join(out_folder))
    bbox_images = sorted(image_list, key=lambda x: int(x.rstrip('.jpg')))
    time_list = []
    for i in tqdm(range(length)):
        ...
        with torch.no_grad():
            ...
            bbox_image = imread(os.path.join(out_folder, bbox_images[i]))
            bbox_image = cv2.resize(bbox_image, (width, height))

            cv2.putText(frame, "Count:" + str(count), (80, 80), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 5)
            # res1 = np.hstack((ori_frame, kpoint_map))
            res1 = np.hstack((ori_frame, bbox_image))
